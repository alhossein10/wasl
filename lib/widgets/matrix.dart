...
class MatrixState extends State<Matrix> with WidgetsBindingObserver {
                ...
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  final lockContext =
                      FluffyChatApp
                          ...
                          .currentContext ??
                      context;
                  AppLock.of(lockContext).onLoginSuccessful();
                });
              });
  ...
  void _registerSubs(String name) {
    ...
    onRoomKeyRequestSub[name] ??= c.onRoomKeyRequest.stream.listen((
      ...
      final isOwnUserRequest = requestingDevice.userId == c.userID;
      final isOtherDevice = requestingDevice.deviceId != c.deviceID;
      final isShareable = !requestingDevice.blocked;
      if (isOwnUserRequest && isOtherDevice && isShareable) {
        Logs().i(
          '[Key Request] Forwarding room key to own device ${requestingDevice.deviceId} (verified=${requestingDevice.verified})...',
        );
    ...
    onLoginStateChanged[name] ??= c.onLoginStateChanged.stream.listen((state) {
      ...
      if (state == LoginState.loggedIn) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!mounted) return;
          AppLock.of(context).onLoginSuccessful();
        });
      }
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        AppLock.of(context).onLoginStateChanged();
      });
    });
