...
class Login extends StatefulWidget {
  ...
  /// When true, the app bar back button is hidden (e.g. when used from
  /// DirectLoginPage with a fixed homeserver).
  final bool hideBackButton;

  const Login({required this.client, this.hideBackButton = false, super.key});
...
class LoginController extends State<Login> {
  ...
  void login() async {
    ...
    try {
      await _enforceHomeserverPolicy();
  ...
  Future<void> _enforceHomeserverPolicy() async {
    if (!AppConfig.strictSingleHomeserver) return;
    if (widget.client.homeserver != AppConfig.forcedHomeserverUri) {
      await widget.client.checkHomeserver(AppConfig.forcedHomeserverUri);
    }
  }
  ...
  void _checkWellKnown(String userId) async {
    if (AppConfig.strictSingleHomeserver) {
      try {
        await _enforceHomeserverPolicy();
