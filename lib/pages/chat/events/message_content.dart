...
class MessageContent extends StatelessWidget {
  static final Set<String> _autoRequestedSessionEventIds = <String>{};
  ...
  void _autoRequestKeyIfPossible() {
    if (event.content['can_request_session'] != true) {
      return;
    }
    ...
    final dedupeKey = '${event.room.id}|$eventId';
    if (!_autoRequestedSessionEventIds.add(dedupeKey)) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final localizedEncryptedFallback = event.calcLocalizedBodyFallback(
      MatrixLocals(l10n),
    );
    final encryptedLabel =
        localizedEncryptedFallback.trim().isEmpty ||
            localizedEncryptedFallback == EventTypes.Encrypted
        ? l10n.encrypted
        : localizedEncryptedFallback;

    _ButtonContent encryptedMessageButton() {
      _autoRequestKeyIfPossible();
