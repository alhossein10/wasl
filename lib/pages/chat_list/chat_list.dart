...
const Set<String> _elementCallStateEventTypes = {
  'org.matrix.msc3401.call',
  'org.matrix.msc3417.call',
  'org.matrix.msc3401.call.member',
  'org.matrix.msc3417.call.member',
  'm.call.member',
};

bool isElementCallRoom(Room room) {
  if (room.isSpace) return false;
  final createType = room
      .getState(EventTypes.RoomCreate)
      ?.content
      .tryGet<String>('type');
  if (_elementCallRoomTypes.contains(createType)) {
    return true;
  }
  if (_elementCallStateEventTypes.any(room.states.containsKey)) {
    return true;
  }
...
class ChatListController extends State<ChatList>
  ...
  bool Function(Room) getRoomFilterByActiveFilter(ActiveFilter activeFilter) {
    bool isRegularDirectChat(Room room) => room.directChatMatrixID != null;

    switch (activeFilter) {
      case ActiveFilter.allChats:
        return (room) =>
            !room.isSpace &&
            isRegularDirectChat(room) &&
      ...
      case ActiveFilter.groups:
        return (room) =>
            ...
            !isElementCallRoom(room);
      case ActiveFilter.calls:
        return isElementCallRoom;
      case ActiveFilter.unread:
        return (room) => room.isUnreadOrInvited && !isElementCallRoom(room);
      case ActiveFilter.spaces:
        return (room) => room.isSpace;
    }
  }

  /// Hide default Matrix.org welcome/official room from chat list.
  static bool isDefaultMatrixOrgRoom(Room room, String displayName) {
    final lower = displayName.trim().toLowerCase();
    if (lower == 'matrix.org') return true;
    if (lower.startsWith('matrix.org (official')) return true;
    return false;
  }
  ...
  static const String _serverStoreNamespace = 'im.wasl.search.server';
